<?php
  ...
  This particular script only reports on who the undrafted players are for this season (or any season you supply in the URL), sorted by ESPN Rank.
  ...
  */
  ...
  $player_query="select * from Players where Season=$season and Players.ID not in (select Player from DraftResult where Season=$season) order by ESPNRank";
...
*/
  ...
  print "<TR><TD>Name</TD><TD>Position</TD><TD>Real<BR>Team</TD><TD>ESPN<BR>Rank</TD>"; 
  foreach ($statarr as $statstr) {
    print "<TD>$statstr</TD>";
  }
  print "</TR>\n";
  while ($player_row=mysql_fetch_array($player_result)) {
    ...
    print "<TD><A HREF=http://www.matthearn.com/fantasybaseball/draftplayer.php?player_id=".$player_row['ID']."&season=$season&league=$league>".$player_row['Name']."</A></TD>";
    print "<TD>".$player_row['PosString']."</TD>";
    print "<TD>".$player_row['RealTeam']."</TD>";
    print "<TD>".$player_row['ESPNRank']."</TD>";
    foreach ($statarr as $statstr) {
      print "<TD>".$player_row[$statstr]."</TD>";
    }
    print "</TR>\n";
